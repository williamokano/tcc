...
Como um dos principais pontos a serem trabalhos, como trabalho futuro se faz necessário que a biblioteca aqui descrita seja utilizada em salas de aula. Elaborar algum método de avaliação se a assimilação pela utilização da biblioteca e verificar se ela realmente gera os resultados esperados.

Como dito na Seção ~\ref{lbl:linguagem_programao_c}, os dados obtidos para a pesquisa foram provenientes de um grupo de programação online. Como sugestão para trabalhos futuros, fazer um levantamento destes dados utilizando as faculdades e universidades brasileiras de ensino de Ciência da Computação, Sistemas de Informação ou cursos similares poderia gerar valores mais precisos sobre a real utilização da linguagem C como linguagem de cursos introdutórios de programação.

Outro ponto interessante seria a adição de novas funcionalidades de desenho, como a possibilidade de desenhar elipses, arcos, linhas curvas e textos. Com essas novas funcionalidades o usuário teria um novo leque de funcionalidades, como por exemplo, a criação, de forma bem simplificada, gráficos de barras e de linhas. A habilidade de poder se expressar por texto, além das formas geométricas, seria uma excelente adição para os próximos trabalhos.
