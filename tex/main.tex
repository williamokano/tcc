...
\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.53]{images/pesquisa_linguagem_tcc.png}
...
\begin{lstlisting}[caption={Inicializando a biblioteca},label={lst:inicializando_biblioteca},language=C]
#include <stdio.h>
#include <graphics.h>
...
Assim como podemos definir a cor de preenchimento de um objeto, também é possível obtermos a cor de preenchimento atual. Esta função se faz útil quando, por exemplo, queremos criar um objeto de uma nova cor mas queremos manter a cor antiga, porém não é conhecido a cor de preenchimento atual. Assim como visto no \ref{api_definirCor}, como a função \textif{definirCor} recebe 3 argumentos de entrada, a saída é um ponteiro representando um array de inteiros de tamanho 3, representando as cores RGB.
...
A saída do método é um array contendo 3 posições e seus valores é descrito conforme a lista abaixo:
...
Alguns objetos podem ter seu tamanho alterado. Para que tenha efeito, a função \textit{definirTamanho} deve ser invocada anteriormente a uma função de desenho de forma. Após chamado o método de definir o tamanho de uma forma, todas os objetos desenhadas após a chamada serão desenhadas com o novo tamanho definido. Atualmente apenas o objeto \textit{ponto(int posX, int posY)} pode ter seu tamanho alterado.

A função que define o tamanho de uma forma tem o seguinte protótipo:

\begin{lstlisting}
void definirTamanho(int espessura);
\end{lstlisting}
...
Um polígono convexo é qualquer polígono que, dados 2 pontos A e B quaisquer interiores a este polígono, o segmento de reta determinado por esses 2 pontos A e B deverá inteiramente estar contido no interior do polígono. Alguns exemplos de polígonos convexos são quadrados, retângulos, triângulos, etc.
